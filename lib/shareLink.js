...
/** Clipboard token for iOS deferred open after App Store install. */
export function buildClipboardHandoffToken(code, lane = 'x') {
  const value = String(code || '').trim();
  if (!value) return '';
  const resolvedLane = normalizeShareLane(lane);
  return `fms_code=${value}&fms_lane=${resolvedLane}`;
}
